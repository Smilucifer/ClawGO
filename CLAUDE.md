...
**Resolved (2026-07-08) — run `npm run rust:test`:** Rust tests used to fail with `STATUS_ENTRYPOINT_NOT_FOUND (0xc0000139)` at load, before `main`, in **any** shell (the old "runs fine via cmd.exe" note was stale — it failed under cmd.exe too). Root cause: `tauri-plugin-dialog` statically imports `comctl32.dll!TaskDialogIndirect`, which exists only in Common-Controls **v6** (WinSxS). The default `C:\Windows\System32\comctl32.dll` is **v5.82** and lacks that export. The main `ClawGO.exe` gets a Common-Controls v6 manifest from `tauri-build`, but `cargo test` binaries do not, so the loader bound v5.82 and the process died at load. Fix: `scripts/rust-test.mjs` (npm `rust:test`) compiles test binaries with `cargo test --no-run`, embeds `scripts/common-controls-v6.manifest` into each via the Windows SDK `mt.exe`, then runs them. This was chosen because cargo has no link-arg scope that reaches lib unit tests without also hitting the bin: `rustc-link-arg-tests` misses lib unit tests (only covers `[[test]]` integration targets), and broad `rustc-link-arg` collides with tauri-build's own bin manifest (`CVT1100 duplicate resource`). Forward args like a normal `cargo test`, e.g. `npm run rust:test -- --lib storage::memos`. Raw `cargo test` still fails at load — always go through `npm run rust:test`.
