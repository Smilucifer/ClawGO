...
**Current phase:** Phase 10+ (v5.5.1, 2026-06-18). 委员会直播修复(跨重启恢复进度/卡片独立运行按钮/解析误报弱化/prompt 精简/排版+关键 chip) + invest Dashboard 优化(A 股 T+1 冻结/持仓明细表 16 列/KPI 卡重构/评级新鲜度/归档文件名带名称) + 全模块金额精度统一 3 位 + 全分支审查抓到 2 个真实 bug(同日买卖/UTC 日期错配)。See `docs/changelog.md`.
