...
| 10+ (v5.4.3) | 宏观指标 Tencent/AkShare 集成+委员会 L4 Officer 移除+Sanity Check 简化+8 项 simplify: fetch_csi300 腾讯 K-line fallback/fetch_cgb_10y AkShare fallback/3 新指标(limit_up_count+limit_down_count+two_market_volume)/tencent_quotes IndexQuote+fetch_csi300_kline+compute_vol20 共享函数/international BondYield10y+MarketStats+RPC 方法/akshare_market.py 新 provider/ALL_INDICATORS 15 全覆盖/L4 Officer 角色移除(orchestrator/parser/roles/cli_executor/前端 5 文件)/5 角色→4 角色/Pipeline 8→7 步/CIO Sanity Check 4 Gates→2 Gates+Fallback/SanityCheckResult gate3+gate4 字段移除/Simplify: vol20 去重+batch HTTP+match arms 分离+Python start_date+market_stats 失败语义+&[f64] 签名+测试简化 | [done] |
