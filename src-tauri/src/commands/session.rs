...
pub(crate) async fn start_session_impl(
    ...
    // Group chat non-executor participants (planner, custom roles): disable all
    // plugins, hooks, and MCP servers to prevent skill-driven behavior and
    // external tool access.
    let disable_hooks_plugins_mcp = is_group_chat && permission_mode_override.is_some();
    ...
    let empty_mcp: std::collections::HashMap<String, serde_json::Value> = std::collections::HashMap::new();
    let managed = crate::agent::provider_claude_config::ManagedConfig {
        mcp_servers: if disable_hooks_plugins_mcp { &empty_mcp } else { &user_settings.mcp_servers },
        hooks: if disable_hooks_plugins_mcp { &empty_hooks } else { &user_settings.hooks },
        enabled_plugins: if disable_hooks_plugins_mcp { &empty_plugins } else { &user_settings.enabled_plugins },
    };
    ...
    // When disable_hooks_plugins_mcp is true, force write to clear native plugins even if managed is empty.
    ...
    let managed_settings_path = if provider_config.is_none() && has_managed_configs {
        match crate::agent::provider_claude_config::write_managed_settings(&run_id, &managed, disable_hooks_plugins_mcp) {
            Ok(path) => {
                log::info!(
