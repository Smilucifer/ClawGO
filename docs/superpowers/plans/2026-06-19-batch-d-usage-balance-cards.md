...
Read `src-tauri/src/models.rs:493-519`,确认 `BalanceHelperSettings` 字段、derive(应有 Serialize/Deserialize/Default、`#[serde(default)]`)与 camelCase 设置。

- [ ] **Step 2: 加三字段**

在 `BalanceHelperSettings` 的 mimo 字段之后、`auto_refresh_secs` 之前插入:
...
(若结构体未用 `#[serde(rename_all = "camelCase")]`,确认前端按 snake_case 读;沿用现有 mimo 字段的命名风格——mimo 字段是 `mimo_service_token` 等 snake,故前端读 snake。保持一致。)
...
- [ ] **Step 1: 确认 reqwest cookies feature**

Read `src-tauri/Cargo.toml` 的 reqwest 依赖行。若未含 `"cookies"`,加上(如 `reqwest = { version = "...", features = [..., "cookies"] }`)。`cargo check` 确认。
...
- Modify: `src-tauri/messages/en.json`、`zh-CN.json`
...
Read `src/routes/usage/+page.svelte:486-685`(Balance Card、DeepSeek/小米面板、网格 class、凭据保存函数如 `saveMimoCredentials`)与 `src/lib/api.ts:480-485`(`refreshBalanceStatus` 类型)。

- [ ] **Step 2: 扩 api.ts source 枚举**

`src/lib/api.ts:480-485`:把 `refreshBalanceStatus` 的 source 参数类型从 `"all" | "deepseek" | "mimo"` 改为 `"all" | "deepseek" | "mimo" | "packyapi"`。

- [ ] **Step 3: 加 PackyAPI 面板**

在 DeepSeek 面板(`:516-557`)之后、小米面板之前插入 PackyAPI 面板。布局:把外层网格从 `md:grid-cols-2` 改为 `md:grid-cols-3`(DeepSeek/PackyAPI 各一列),小米面板 class 从 `md:col-span-2` 改为 `md:col-span-3`(独占整行)。PackyAPI 面板用绿色系渐变区分,复用 `balanceStatusText("packyapi")`:
...
```svelte
      ...
      <div class="rounded-xl bg-gradient-to-br from-emerald-500/10 ... p-4 space-y-2">
        ...
        <div class="text-sm">{packy.text}</div>
...
(具体 class 与 DeepSeek/小米面板对齐;`packy.dotClass`/`packy.text` 来自 `balanceStatusText` 返回——确认其返回结构,Step1 已 Read。)

- [ ] **Step 4: 加凭据 state + 保存函数**

仿 `saveMimoCredentials`:加 `packyapiSession`/`packyapiItoken`/`packyapiUserId` 的 `$state`,从 `getUserSettings().balance_helper` 初始化;`savePackyapiCredentials` 调 `api.updateUserSettings({ balance_helper: { packyapi_session, packyapi_itoken, packyapi_user_id } })` 后 `refreshBalanceStatus("packyapi")`。
...
```json
  ...
  "settings_balance_save": "保存",
```
en.json:
```json
  "settings_balance_packyapi_creds": "PackyAPI Credentials",
  "settings_balance_save": "Save",
```
(若 `settings_balance_save` 已存在则复用,先 grep。)
...
```bash
git add src/routes/usage/+page.svelte src/lib/api.ts src-tauri/messages/en.json src-tauri/messages/zh-CN.json
