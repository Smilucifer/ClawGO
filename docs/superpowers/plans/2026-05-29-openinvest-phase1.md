...
| Modify | `src-tauri/src/commands/memos.rs` | Add scope-aware memory commands |
...
Add indexes for scope filtering after the CREATE TABLE:
...
- [ ] **Step 1: Add list_memories command**

```rust
#[tauri::command]
pub fn list_memories(scope: Option<String>) -> Result<Vec<crate::storage::memory_store::MemoryItem>, String> {
    crate::storage::memory_store::list_memories(scope.as_deref())
}
```

Note: The old `list_memos(scope: MemoScope)` remains for backward compatibility in the GlobalMemoPanel. The new `list_memories` command is for the `/memory-mgmt` page.
...
In `lib.rs`, find the memory DB init block and add after it:
...
Find the `navItems` array. Insert `/invest` after `/plugins` and `/memory-mgmt` after `/settings`:
...
In the template's icon if/else chain, add a new block for `"trendingUp"`:
...
<script lang="ts">
  ...
  async function loadMemories() {
    loading = true;
    try {
      memories = await invoke("list_memories", { scope: scopeFilter });
    } catch (e) {
