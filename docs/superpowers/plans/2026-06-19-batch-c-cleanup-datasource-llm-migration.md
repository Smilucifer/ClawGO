...
- 删除/新增任何 UI 文案都要同步 `src-tauri/messages/en.json` + `zh-CN.json` 并通过 `npm run i18n:check`。
...
- Modify: `src-tauri/messages/en.json`、`zh-CN.json`(删相关 key)
...
- Modify: `src-tauri/messages/en.json`、`zh-CN.json`(按钮/表单文案)
...
`add_holding`、`update_holding`、`delete_holding`、`set_initial_cash`、`update_cash`、`save_verdict`、`save_pnl_snapshot`、`save_event`、`get_event_sources`、`save_event_source`、`is_trading_day`、`get_scheduler_logs`、`get_strategy`、`get_daily_bars`、`sync_trade_calendar`、`get_tracking_status`、`list_all_tracking`、`generate_daily_report`、`list_daily_reports`。
...
删除 `src-tauri/src/lib.rs` 对应注册行:404、405、406、414、415、417、419、422、424、425、426、427、428、435、437、459、460、474、475。(行号以删除前为准;逐行按命令名匹配删除,避免错删。)
...
替换 `event_analyzer.rs:137-158` 的 chat_stream 段为 `cli_complete(system_prompt, &items)`,同样删除 `client`/`config` 参数;更新 `analyze_pending_events` 签名(删 `&llm_client, &llm_config`)与 `runner.rs:77-83` 调用点(不再需要 `build_scan_clients`)。

- [ ] **Step 4: 改 scheduler runner 与 scan_events 命令不再构造 LLM client**

`src-tauri/src/invest/scheduler/runner.rs:77`:删除 `build_scan_clients()` 调用,改为只取 tushare(event_scan 需要 tushare),event_analyzer 不再需要 client/config 参数。
...
`build_committee_config`(`commands/invest.rs:796`)目前从 `InvestLlmConfig` 读 `selected_provider`/`model_override`/`debate_rounds`/`timeout_secs`/`max_concurrent_symbols`。新增一个轻量结构(如 `CommitteeTuning`,持久化到 `~/.claw-go/invest/committee_tuning.json` 或并入 `UserSettings`)承载这几个字段,改 `build_committee_config` 读它。保留 `try_parse_provider_id` 给该结构用。

> 实现时先 Read `build_committee_config` 全文确认它消费的字段,再决定新结构形状。前端 `ProviderConfigPanel` 若仅控制这几个字段,改为读写新结构。
...
- `src/lib/components/invest/SystemDatasourceTab.svelte`:删 LLM provider 部分(`:13-17, 33-44, 52-57`)及 `get_llm_config` 调用。
...
```bash
git add src-tauri/src/commands/invest.rs src-tauri/messages/en.json src-tauri/messages/zh-CN.json
