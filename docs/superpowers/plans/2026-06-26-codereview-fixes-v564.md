...
- ✅ **H-sec-4** `commands/diagnostics.rs:404/426/480` — SSH `user@host` 无 `--` 终止符、不拒绝 `-` 前缀，argv-flag 注入。**已修**：`test_remote_host` 拒绝 user/host 以 `-` 开头；两处 ssh 命令在 target 前加 `--` 终止符。
...
- ✅ **H-state-1** `agent/session_actor.rs:1684-1694` — `SessionInit` 持久化 session_id/`conversation_ref` 失败仅 warn，重启后 `claude --resume` 无 session id 无法恢复。**已修**：重试 3 次（间隔 100ms），全失败升级 `log::error!` 提示不可 resume。
- ✅ **H-state-2** `agent/session_actor.rs:2287-2294` — 终态 `update_status` 写失败仅 log，盘上 meta 仍为 `Running`，history/UI 状态不一致。**已修**：紧密重试 3 次（emit_state 同步，不引入阻塞 sleep），全失败升级 `log::error!`。
- ✅ **H-state-3** `agent/session_actor.rs:1273-1286` — `pending_interrupt` 在 `write_json_line().await?` 之前置位，写失败提前返回但 flag 已置，后续真实 `failed` 被改写成 `idle`，遮蔽 CLI 失败。**已修**：write 成功后才置 `pending_interrupt` 与插入 `control_waiters`，避免写失败留下错误 flag/孤立 waiter。
- ✅ **H-utf8** `agent/stream.rs:209-214` — 旧 pipe 路径 8KiB 固定 buf + `from_utf8_lossy`，多字节 UTF-8 跨 chunk 被替换成 U+FFFD；改 `read_line`/增量解码。`executor/codex.rs:62-64` 把 stdout `Err` 当 EOF。**已修**：stream.rs 用 `pending` 字节缓冲跨读取保留不完整多字节序列，解最长有效 UTF-8 前缀，stdout 读错误改为 log 而非静默 break；codex.rs 循环显式区分 EOF/Err，读错误 log。
- ✅ **H-gc-1** `storage/group_chats.rs:232-233` — detach 用 `{run_id}.meta.json`，但 `save_participant_meta` 存的是 `{participant_id}.meta.json`（不同 UUID）；每次 detach 留孤儿元数据，重 attach 读到陈旧 cursor。**已修**：retain 前捕获被移除 participant 的 `id`，按 participant_id 删 meta（保留 C4 的 validate + 不吞错误）。
- ✅ **H-gc-2** `group_chat/orchestrator.rs:541-558` + `storage/group_chats.rs:300-307` — 失败 turn 复用 turn_id 追加 `responses: vec![]`，`list_turns_jsonl` 按 id 保留最后一行，覆盖已落盘的部分响应（显示为零响应）。**已修**：`list_turns_jsonl` dedup 时，新行 responses 空而旧行非空则保留旧 responses，仅采用新行 completed_at。
...
- ✅ **H-data-2** `invest/macro_refresh.rs:104-153` — 复合返回值 `is_valid` 只查 `close`，MiniQmt 有 `close` 但 `vol20=None` 时视为成功，Tushare 永不再试，vol20 行与 close 行 age/source 漂移。**已修**：validity 同时要求 vol20 为 `Some(finite)`，缺 vol20 时继续走链 fallback。
- ✅ **H-data-3** `invest/macro_refresh.rs:221-276` — `margin`/`shibor` 取最新行不校验，字段为 0 也写入，source 硬编码 `tushare`。**已修**：`margin.rzye` / `shibor.on` 为 0 或非有限时返回 Err（这两值无合法 0），不再写入垃圾数据。（source 硬编码因两函数无 chain、确为 tushare 专用，保留。）
- ✅ **H-fe-race** 四个单例 store 在 await 后无序号/abort 守卫，快速切换用旧响应覆盖新的：`character-memory-store.svelte.ts:12-21`、`doctor-store.svelte.ts:13-29`、`user-memory-store.svelte.ts:11-19`、`claude-usage-store.svelte.ts:10-24`。**已修**：四个 store 各加 `#loadSeq`/`#runSeq`/`#refreshSeq`（参照 memo-store 模式），await 后比对序号，stale 响应直接 return 不覆盖；doctor `clear()` 也自增序号。
...
### 原子写 / 并发锁缺失
- ✅ `storage/mcp_registry.rs:1159`（重写 `~/.claude.json`，与活跃 CLI 写竞争）、`:656/672/690`（`~/.codex/config.toml`）— 非原子无锁。**已修**：新增 `CONFIG_WRITE_LOCK`（std Mutex）+ `write_config_atomic`（tmp+rename），四个写入点（claude.json toggle + codex add/remove/toggle）统一加锁原子写。
- ✅ `storage/cli_config.rs:72-107` — `~/.claude/settings.json` load→mutate→write 非原子无锁，且与运行中 CLI 共享。**已修**：加 `CLI_CONFIG_LOCK` 串行化 load-mutate-write；改 tmp+rename 原子写，0o600 在 rename 前设（消除可读窗口）。
- ✅ `agent/provider_claude_config.rs:231/279/310` — 合并 settings（含 token）非原子写，且未设 `0o600`。**已修**：新增 `write_secure_config`（tmp+0o600+rename），三个写入点（provider/mcp/managed）统一使用。
- ✅ `storage/artifacts.rs:32`、`storage/plugins.rs:681/718`（SKILL.md）— 非原子写。**已修**：新增公共 `storage::write_atomic_string`（tmp+rename），artifacts.json 与两处 SKILL.md 写入改用之。
- ✅ load→mutate→save 无锁丢改动：`storage/favorites.rs:68-144`、`storage/settings.rs:544-583`（`update_user_settings`）、`:904-918`（`update_agent_settings`）。**已修**：settings 加 `SETTINGS_LOCK`（6 函数）；favorites 加 `FAVORITES_LOCK`（add/remove/update_tags/update_note 四函数）。

### panic / 鲁棒性
- ✅ `commands/diagnostics.rs:262`（`&s[..50]`）、`:763`（`&k[k.len()-4..]`）— 按字节切 UTF-8 可能 panic。**已修**：改 char 边界截断（`chars().take(50)` / 取末 4 char）。
- ✅ `storage/run_index.rs:410/420/424/552/609`、`storage/events.rs:178/191/234/244/293` — `lock().unwrap()` 中毒后整模块后续操作全 panic。**已修**：10 处统一改 `unwrap_or_else(|e| e.into_inner())`。
- ✅ `storage/teams.rs:8` — `expect("home dir")` 在 HOME/USERPROFILE 不可解析时 panic 命令线程。**已修**：回退链 dirs_next → USERPROFILE → HOME → 当前目录 + log::error，不再 panic。
- ✅ `storage/runs.rs:339-340/485-486` — `list_runs` 静默丢解析失败的 run，损坏 1 条 meta 即在历史消失。**已修**：`list_runs` 与 `list_all_run_metas` 的 read/parse 改 match，失败时 `log::warn!` 带路径，不再静默吞。
- ✅ `invest/committee/orchestrator.rs:1757/1810` — spawn 任务里 `expect("semaphore closed")`，未来重构可致 worker panic、`JoinError` 被吞、前端收不到 `SymbolAborted`。**已修**：两处改 match 返回 `Err(String)`，不再 panic worker。

### Windows / 功能性
- ✅ `commands/agents.rs:157` — Windows 上 `target` 未 canonicalize 而 `parent_to_check` 已 canonicalize（`\\?\` 前缀），前缀比较恒不匹配，**project-scope agent CRUD 在 Windows 整体失效**。**已修**：改为 canonicalize agents 目录的最近存在祖先，用组件级 `Path::starts_with` 比较（替代字符串前缀），file_name 已有 traversal 校验兜底。

### invest 解析 / 计算
- ✅ `invest/indicators.rs:108-111` — 价格分位用首个等值索引，平窗塌缩到 0%、封顶 (n-1)/n；改 tie-rank。**已修**：改 `(lower + 0.5*equal)/n*100` 中点 tie-rank，平窗→50%、最高→~100%；更新单测（30 在 [10..50] 现为 50% 而非 40%）。⚠️ 改变分位语义，下游分位阈值判断行为随之变化（更统计正确）。
- ✅ `invest/committee/parser.rs:289-297`（continuation 合并污染 KEY 值）、`:525`（pnl_pct 未强制符号）、`:458`（KEY_DATA 遇任意冒号即终止丢字段）、`:371-380`（`parse_leading_f64` 不识 `,`/`，`/`。`）。**部分已修**：(b) pnl_pct 遇 亏/loss/前导`-` 强制负号；(c) KEY_DATA 终止条件改 `is_structured_key_line`，列表项可含冒号；(d) `parse_leading_f64` 扩展分隔符（`，。;；、`）+ 英文逗号千分位剥离。(a) continuation 合并污染**暂留**——涉及 merge 核心逻辑且有单测锁定现行为，需单独评估。
- ✅ `storage/invest/macro_cache.rs:184-195` — `is_stale` 纯墙钟，周末把最新交易日数据误判为 stale。**已修**：周末（CST Sat/Sun）且数据在最近周五之后抓取则视为新鲜，不再按墙钟分钟判 stale。
- `invest/scheduler/runner.rs:268-285/334-340` — 热路径在 runtime worker 上做同步 SQLite/FS I/O，应 `spawn_blocking`。**暂留**（性能优化，非正确性 bug）。

### group_chat 其它
- ✅ `orchestrator.rs:966-997/1424-1447` — `@mention` 用 `trim_start_matches('@')`，带尾标点（`@Alice,`/`@Alice.`）查不到 participant，命令降级 Fanout、auto-chain 断链。**已修**：新增 `strip_mention`（剥前导 @ + 尾部标点，保留 `._-`），find_participant/find_participant_unique/extract_first_mention 统一使用。
- ✅ `orchestrator.rs:1429-1431` — `extract_first_mention` 在循环内用 `?` 碰到 `preview=None` 提前返回，应 `continue`。**已修**：改 `let Some(text) = ... else { continue }`。
- 只增不减的全局 HashMap：`orchestrator.rs:38-41`、`storage/group_chats.rs:14`、`memory_extraction.rs:39/44`（`DAILY_EXTRACTION_COUNT` 还会把昨天配额带给同 id 新 group chat）。**暂留**——内存缓慢增长，非正确性 bug；清理需 `Arc::strong_count` 守门防竞态丢锁，单独评估。
- ✅ `group_chat/memory_dream.rs:34-42/127` — `mark_dream_time` 用 `let _ = fs::write`，写失败后 `should_run_dream` 恒 true，每 tick 重跑快照+衰减。**已修**：改原子写（tmp+rename）+ 写失败 `log::error!`，问题可见且无半截文件。
- 🔶 `agent/session_actor.rs:1283-1286`（孤立 oneshot sender 留在 control_waiters）、`:1291-1305/2184-2192`（`child.wait()` 无超时，Windows kill 失败则 actor 卡死）、`:1647-1664`（记忆抽取 spawn 丢 JoinHandle 不可取消）、`:559-572/697-702`（turn index 写失败前已自增，留空洞）。**部分已修**：两处 `child.wait()` 加 5s timeout（Windows kill 失败不再卡死 actor）。其余（孤立 waiter GC、抽取 JoinHandle、turn index 延迟分配）**暂留**——需更大改动，风险/收益比偏低。
...
## CodeEditor 已知问题（独立，非本次回归）✅ 已修

`src/lib/components/CodeEditor.svelte:155` 行内注释含字面量 `<style>` token，骗过 svelte-language-tools 分词器，使 `<script>` 提前闭合 → explorer/memory 两页报 "no default export"。runtime 编译器本身能接受。**已修**：把注释里的 `<style>` 改成 `the style block`，`svelte-check` 错误 3 → 0（已验证）。
