...
- ✅ load→mutate→save 无锁丢改动：`storage/favorites.rs:68-144`、`storage/settings.rs:544-583`（`update_user_settings`）、`:904-918`（`update_agent_settings`）。**已修（settings 部分）**：新增 `SETTINGS_LOCK`，6 个 mutate 函数（update_user/agent_settings、update_embedding_config、save_web_server_config、set_web_server_enabled、save_web_server_partial_disable）入口加锁串行化。favorites 待后续同样处理。
