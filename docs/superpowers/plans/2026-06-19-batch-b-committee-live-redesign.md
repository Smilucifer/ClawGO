...
- 任何新增/改动的 UI 文案必须同时更新 `src-tauri/messages/en.json` 与 `zh-CN.json`,并通过 `npm run i18n:check`。
...
- Modify: `src-tauri/messages/en.json`、`src-tauri/messages/zh-CN.json`(新增 regime chip 文案键)
...
```css
  ...
  .regime-chip {
    ...
    background: color-mix(in srgb, var(--color-quant, #c084fc) 13%, transparent);
    border: 1px solid color-mix(in srgb, var(--color-quant, #c084fc) 30%, transparent);
    white-space: nowrap;
  }
  .regime-chip.unknown { opacity: 0.45; }
  .regime-chip .rg-tag {
    font-size: 11px; font-weight: 700;
    color: var(--color-quant, #c084fc);
...
在 `src-tauri/messages/zh-CN.json` 与 `en.json` 各加一个键(若 `invest_regime_unknown` 已存在则复用,先 grep 确认):
...
- Modify: `src-tauri/messages/en.json`、`zh-CN.json`(字段标签键)
...
```ts
  type ParsedFields = NonNullable<SymbolProgress['result']>['rounds'][number]['parsed'];
...
`src-tauri/messages/zh-CN.json` 新增:
```json
...
```
`src-tauri/messages/en.json` 新增对应英文:
```json
