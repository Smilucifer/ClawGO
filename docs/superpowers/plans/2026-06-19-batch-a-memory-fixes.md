...
- [ ] **Step 6: 更新三处 `spawn_actor` 调用点**

`src-tauri/src/commands/session.rs:888-902`(主 start_session)— 在末位 `auto_approve_mcp,`(注意此处用的是函数参数 `auto_approve_mcp`)之后加 `is_group_chat,`(该函数已有 `is_group_chat: bool` 参数,见 `session.rs:549`):
...
`src-tauri/src/web_server/dispatch.rs:850-862`— 该调用是 `start_session` 包装,确认它传入的 `is_group_chat` 实参为 `false`(`dispatch.rs:861`)。若它调用的是 `start_session`(已带 is_group_chat 参数)而非直接 `spawn_actor`,则无需改 spawn_actor 调用,只需确认透传。核对后:若为直接 `spawn_actor` 调用则比照加 `false`;若经 `start_session` 则无需改。
...
```bash
git add src-tauri/src/agent/session_actor.rs src-tauri/src/commands/session.rs src-tauri/src/web_server/dispatch.rs
