...
3. **候选深度因子**：技术因子需 60 根 K 线，仅对入围候选逐只拉 K 线，用 `buffer_unordered` 限速慢拉(盘后时间充足)。四因子(sentiment/capital/technical/catalyst)计算复用现有 `compute_*` 逻辑。
4. **落地缓存表** `premarket_factor_cache`(SQLite，`storage/invest/`)：
   `{date, symbol, name, change_pct, amount, sentiment_score, capital_score, technical_score, catalyst_score, missing_factors, cached_at}`，主键 `(date, symbol)`。

### B. 生成 job `premarket_report` 改为纯读缓存
- `report.rs` 的 `collect_pool()` → 新 `collect_pool_from_cache(date)`：读 `premarket_factor_cache` 当天行 → 直接 `score()` 组装 SABC。无网络调用。
- **缓存缺失兜底**(盘后 job 没跑/失败)：现场触发一次 A 的粗筛+慢拉(耗时较长)，报告标注"实时拉取(缓存缺失)"。
- **名称解析**：tick/daily 无中文名时用 `stock_industry` 表批量查，回退代码。
...
改动：
- **新增 scheduler job `sentiment_collector`**，cron `每 1 小时`(如 `0 0 8-22/1 * * 1-5` 量级)：
...
- **生成时 01 段读当天累积全量**：放宽 `build_news_block_for_ai` / `list_recent_sentiment` 的 40 条上限，改按"当天全部已归一化 sentiment"聚合，AI 点评反映一整天新闻量。
...
### 改动
1. **海外 6 指标改 akshare**：
   - Python `providers/akshare_market.py` 新增覆盖 VIX/美10Y/美元指数/金/油/汇率的函数(具体 akshare 接口名实现时验证覆盖)。
...
- 后端：`premarket_cache` 粗筛+缓存写入单测(mock tushare 批量返回)；`collect_pool_from_cache` 读取单测；注册表 `Category::Overseas → Akshare`、删 `SourceId::Yahoo` 后 `cargo build` + 现有源链测试通过。
...
后端数据层(6/8/9)先行 → provider/健康检查(9) → LLM provider(7) → 前端(1/2/3/4/5)。
