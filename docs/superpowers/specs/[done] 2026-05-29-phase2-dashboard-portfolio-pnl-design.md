...
13 项确认发现已修复（`a1a41f6`），1 项驳回。关键修复：

- **tushare_token 未持久化** — `update_user_settings` 缺少该字段的 apply 调用
- **block_on panic** — PnL cron 在 tokio async 内调 `Handle::block_on()`，改为 async/await
- **Strategy targets 双重编码** — SQLite TEXT → serde String → JSON string，改为 Vec + 自定义反序列化
- **DailyBar/StockBasic 缺 camelCase** — 前端收到 snake_case keys
- **sellStock 无超卖校验** — 超卖时持仓删除 + 虚增现金
- **openConvert 重复持仓** — watch→hold 转换未删除原 watch 记录
- **PnL 快照无去重** — 应用重启产生重复记录，改为 upsert
