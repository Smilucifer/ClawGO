...
- **投资委员会** —— 多角色 LLM 辩论(宏观 / 量化 / 风控 / CIO),分两轮对每只标的给出结构化裁决,支持研究与实盘双模式。每个角色通过 Claude CLI 执行,provider / 模型来自委员会调参与凭据配置。宏观解读改为全局单卡(定时生成 + 交易时段门禁,含市场广度与赚钱效应),每只标的仅显示行业敏感度。
- **行情与数据** —— 接入 Tushare、腾讯实时行情、AkShare(经 Python 桥)、东财直连等多个数据源,内置统一的数据源编排层(按指标类别自动选源、判空降级、记录命中源)。海外指标(DXY/US10Y 走东财,VIX/黄金/原油/美元兑人民币走 AkShare)。支持可选的 miniQMT 本地行情源(开启后 K线/实时报价优先从本地获取,无限频)。内置 RSI/MA、市场 regime、宏观指标缓存(上证指数、涨跌家数、北向资金、融资余额、涨幅>3%家数等 19 项;miniQMT 在线时一次取全 A 市场广度)等技术分析。东财板块数据用于板块强度因子。
- **持仓盯盘** —— 持仓与观察列表、当日盈亏(现金流调整盯市法,正确覆盖新买/加仓/减仓/清仓)、清仓当日保持持仓 + 次日自动转观察等。支持可配置的手续费方案(佣金/印花税/过户费),佣金自动计入盈亏。
- **盘前观察报告** —— 面向下一交易日的图文长报告,数据打底 + AI 精筛:多源舆情聚合成板块、板块资金流入榜 + 拥挤度雷达、五因子(含板块强度)次/切/档观察池分级、多信号候选池(舆情∪主力净流入∪涨幅)、明日主线排序。硬数据确定性打分(评级不受 AI 影响);可选 AI 精筛对 top 标的风险审查,误判过多自动回退纯量化。支持一键生成、PNG/PDF 导出、自定义权重与阈值。工作日收盘后预构建因子缓存,生成时读缓存(十几秒出报告),入口在「委员会 → 盘前观察」。
- **舆情采集** —— 同花顺 / 新浪 / 财联社 / 东财股吧 / 雪球五源舆情统一去重入库 + AI 归一化(摘要、情绪立场、行业映射),每小时自动采集,供盘前报告与委员会催化共读。
- **定时任务** —— 盈亏快照、事件扫描、每日报告、盘前观察报告、盘前因子缓存、舆情采集(每小时)、定期"dreaming"反思。
- **事件追踪** —— 高频 Jin10 事件流采集 + LLM 归一化与扫描。
- **每日盈记(Fortune Journal)** —— 基于天干地支 60 日循环的娱乐化盈亏统计。日历视图、天干/地支分层评分、智能预测(加权合成 0–100 分)、AI 解读、批量录入。纯娱乐性质,不构成投资建议。
