...
- **行情与数据** —— 接入 Tushare、腾讯实时行情、AkShare(经 Python 桥)、全球指数等多个数据源,内置统一的数据源编排层(按指标类别自动选源、判空降级、记录命中源)。支持可选的 miniQMT 本地行情源(开启后 K线/实时报价优先从本地获取,无限频)。内置 RSI/MA、市场 regime、宏观指标缓存(上证指数、涨跌家数、北向资金、融资余额、涨幅>3%家数等 19 项;miniQMT 在线时一次取全 A 市场广度)等技术分析。
