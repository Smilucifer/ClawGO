...
- **盘前观察报告** —— 面向下一交易日的图文长报告,数据打底 + AI 点评:多源舆情聚合成板块、板块资金流入榜 + 拥挤度雷达、S/A/B/C 四因子观察池分级、明日主线排序。硬数据确定性打分(评级不受 AI 影响),支持一键生成、PNG/PDF 导出、自定义权重与阈值。工作日收盘后预构建因子缓存,生成时读缓存(十几秒出报告),入口在「委员会 → 盘前观察」。
